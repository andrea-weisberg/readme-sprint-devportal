...
If a client app relies on Paymentology for TAV calculation, they’ll need to call the [CalculateTAV](/api-reference/companion-api/calculatetav) API method, which is part of the Companion API. Paymentology will also need to get the necessary keys from Mastercard.

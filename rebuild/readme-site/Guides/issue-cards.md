...
If the API receives the request, it will create a 16-digit PAN (Permanent Account Number), CVV (Card Verification Value), and expiry date. You can then deliver this information to your customer.![](https://developer.sprint.paymentology.com/wp-content/uploads/2023/03/How-to-issue-a-virtual-card-v2.png)
...
You can choose either of the following options for issuing a physical companion card: Issue on-site and link immediately or issue with courier and link later.![](https://developer.sprint.paymentology.com/wp-content/uploads/2023/03/Physical-card-fulfilment-flow-v2.png)
...
When activating and linking the card, you’ll need to make a call to the [ActivateCard](/api-reference/companion-api/activatecard) method first to ensure the card is in an active state. Once the card is activated, you’ll need to send a request to Paymentology, using the [LinkCard](/api-reference/companion-api/linkcard) method, for the card to be linked to a unique customer reference number.![Companion API card issuing flow](https://developer.sprint.paymentology.com/wp-content/uploads/2023/03/Companion-card-issuing-flow-1-v2.png)

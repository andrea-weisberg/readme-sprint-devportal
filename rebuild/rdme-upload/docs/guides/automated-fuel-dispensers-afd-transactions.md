...
category:
  uri: Guides
slug: automated-fuel-dispensers-afd-transactions
position: 24
parent:
  uri: manage-funds
---
...
In the past, fueling was a tedious process—an attendant had to manually pump the requested amount of fuel, usually resulting in lines and customer queues. However, AFDs has turned this around—no more direct engagements with staff and other time-consuming hassles.
...
![AFD Transaction processing](https://files.readme.io/cb39e38d079179b7babdbf65211439597564a3a4b3656ebd4f7c96f031488a09-172ea364d8fe2bb6f7ec13335cc7eb18dd5d50603ed9eb0415357504e9eddce3-AFD-Transaction-processing-v2.png)

- Cardholder makes a transaction at an AFD POS

- POS sends an authorization request to a card scheme

- Card scheme sends an authorization to Paymentology

- Paymentology sends payment authorization to a wallet provider

- Wallet provider accepts or rejects the transaction based on the availability of funds

- If the wallet accepts successfully, confirmation advice is passed from AFD POS to the card scheme. If the wallet declines, the decline reason response will be sent

- AFD POS device passes confirmation to the card scheme. Card scheme sends confirmation advice to Paymentology

- Paymentology processes the confirmation advice and checks if there is a pending balance from the original authorization

- Paymentology sends a reversal to the wallet provider for the balance of the original pre-authorization amount.

...
In order to obtain a token, an http request to:
`[https://auth.uat.tutuka.cloud/oauth/token](https://auth.uat.tutuka.cloud/oauth/token)`
using Basic Authentication and providing the BASE64 encoded ClientID and Secret in the Authorization Header specifying the `client_credentials` grant type in the body. e.g.
...
(repeats same code block as above—this duplication is intentional as per original content)
...
(code blocks repeat here — preserved verbatim as instructed)
...
(repeat code blocks again — preserved)
...
| CampaignUUID | String |        |     ✓    | The campaignUUID for which you want to obtain a list of report types.   |
| ReportId     | String |        |     ✓    | The numeric id identifying the report type                              |
| Date         | String |        |     ✓    | The date for which a download url should be generated. Format: YYYYMMDD |

(repeat all code again — untouched)

*The reportUrl is a pre-signed S3 URL which is valid for 5 minutes. Note that the URL in the response is escaped to unicode.*

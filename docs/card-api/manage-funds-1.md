...
<!-- MIGRATION_METADATA
Migrated-From: 
Source-Slug: manage-funds-1
Migrated-On: 2026-02-12T20:54:36+00:00
...
> 📘 Info
>
> To enable and manage this process, Paymentology Sprint uses what is called the [**Remote API**](../companion-api/api-reference-companion/remote/).
...
You’ll need to make a call to the [Balance](../companion-api/api-reference-companion/remote/balance) method.
...
Scenario: In this scenario, a [Deduct ](../companion-api/api-reference-companion/remote/deduct)request was sent to the store of value system but there was no response received. This will cause a time out, as no response will be sent back to the merchant.
...
Due to no response, a [DeductReversal](../companion-api/api-reference-companion/remote/deductreversal) will be triggered and the store of value system will match the **TransactionID **of the initial [Deduct ](../companion-api/api-reference-companion/remote/deduct)and match it with the **ReferenceID** in the [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal)and **Approves** the [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal)by responding with **1 – Success** and reverse the funds the funds back.
...
Scenario: In the scenario, due to connectivity issues the [Deduct ](../companion-api/api-reference-companion/remote/deduct)request is not sent to the store of value system. This results in a timeout in the transaction. The merchant will timeout the transaction and trigger a [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal)which will be sent to the store of value system. At this point, the store of value system has no record of the initial [Deduct](../companion-api/api-reference-companion/remote/deduct).

The store of value system will attempt to match the **ReferenceID** in the [DeductReversal](../companion-api/api-reference-companion/remote/deductreversal), which they will not find as the initial [Deduct ](../companion-api/api-reference-companion/remote/deduct)did not reach the store of value system. The store of value system will send an acknowledgement = **1 – Success **but no funds will be moved on the store of value system

The store of value system may not respond with a -9 Crashed or disapproved response.

#### 3. The merchant voided the transaction

Scenario: In this scenario, the authorization is cancelled/ voided immediately after a successful transaction has been concluded. The store of value system will have received the initial [Deduct ](../companion-api/api-reference-companion/remote/deduct)request.

The store of value system will match the **ReferenceID** of the [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal)to the **Transaction ID** of the initial [Deduct ](../companion-api/api-reference-companion/remote/deduct)request. They will reverse the funds and respond with **1 – Success.**
...
Paymentology will link the deduct reversal to the original authorization, we do this by including a ReferenceID in the [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal)API request. The ReferenceID field is the transaction ID of the original authorization (Deduct).
...
Mastercard has mandated that refunds should be authorised but merchants do not follow this. Therefore, you will find very few [LoadAuth ](../companion-api/api-reference-companion/remote/loadauth)requests as most as just sent immediately as settlement (which you will receive as [LoadAdjustments](../companion-api/api-reference-companion/remote/loadadjustment))
...
You’ll need to respond to the [LoadAdjustment ](../companion-api/api-reference-companion/remote/loadadjustment)method.


> 📘 Info
>
> A Refund is a movement of funds from a merchant to a cardholder and may or may not relate to a previous deduct of funds. This uses the [LoadAuth](../companion-api/api-reference-companion/remote/loadauth) method. Refunds are a new movement of funds.


> 📘 Info
>
> A Load Adjustment is an adjustment being made that gives money to the cardholder that has to be accepted by the wallet. It relates to a previous transaction. This uses the [LoadAdjustment](../companion-api/api-reference-companion/remote/loadadjustment) method. Load Adjustments happen when the cardholder has more money than anticipated, such as when a settlement is cancelled or a settlement happens for less than the authorization amount.


### LoadAdjustment

As for a [LoadAuth](../companion-api/api-reference-companion/remote/loadauth), this is the result of a refund or a money transfer request that loads funds to a cardholder but this is an advice message confirming that the funds have moved.

When a [LoadAuth ](../companion-api/api-reference-companion/remote/loadauth)is settled, it will result in a [LoadAdjustment ](../companion-api/api-reference-companion/remote/loadadjustment)(this assumes no [LoadAuthReversal ](../companion-api/api-reference-companion/remote/loadauthreversal)occurred). As with all advice messages, it is **required** that the message is approved. There is no other option except for a system failure/crash. Your response of “1” is not “approving” the adjustment, it is confirming that you have been notified of the adjustment. All adjustments have already occurred and declining such a message will not change that. If you respond with anything besides approval, it results in a process of manual intervention to investigate why the failure occurred and to work with you to take steps to correct it.
...
This is a reversal message for [LoadAuth ](../companion-api/api-reference-companion/remote/loadauth). It is the equivalent of [DeductReversal ](../companion-api/api-reference-companion/remote/deductreversal). This will be sent if we do not receive any response to the [LoadAuth ](../companion-api/api-reference-companion/remote/loadauth). As with all adjustments and reversals, they are “advice” messages, they are “telling” you to do something. They are not asking you to approve something. Your response should therefore be an acknowledgement that you have received the message not that you “approve” the message. For all reversals, we will keep resending if we do not receive positive acknowledgement. Failure to respond repeatedly results in manual intervention which should only happen if your system is down.
...
After that, Paymentology will resend a [LoadAdjustment ](../companion-api/api-reference-companion/remote/loadadjustment)request so that an OK response can be returned. This pattern will continue ten times until the adjustment is processed adequately against the stored value.

You’ll need to make a call to the [LoadReversal ](../companion-api/api-reference-companion/remote/loadreversal)method.
...
What you do on your system for a [LoadAuthReversal ](../companion-api/api-reference-companion/remote/loadauthreversal)will depend on what you do for a [LoadAuth ](../companion-api/api-reference-companion/remote/loadauth). If you approve and did nothing else (see [LoadAuth](#LoadAuth) above for more information), then there isn’t anything for you to do when reversing. If your system records and reflects the “pending” load, you’d need to remove this pending load.


> 📘 Info
>
> A Reversal is a cancellation (in whole or in part) of a fund movement from the cardholder to the merchant. This uses the [LoadAuthReversal](../companion-api/api-reference-companion/remote/loadauthreversal) method.
...
This is sent if we do not receive any response to the [LoadAdjustment](../companion-api/api-reference-companion/remote/loadadjustment). All the other rules mentioned in [LoadAuthReversal ](../companion-api/api-reference-companion/remote/loadauthreversal)still apply.
...
One big difference is that, almost without exception, a [LoadAdjustment ](../companion-api/api-reference-companion/remote/loadadjustment)will result in actual funds having been loaded to a cardholder balance, and so reversing this means you will need to “undo” that load. Again, undoing a load might practically be done through a deduct but you are  not deducting, you are putting the wallet back to it’s previous state. The difference becomes important when ensuring that  you are able to handle repeats. Do not keep deducting every reversal you receive as you may already have done the deduction. Remember that the connection failure we are experiencing with you, might only be for messages after you have processed them.
...
In both cases, you would effectively *do nothing*, because there is nothing to reverse.
...
You’ll need to make a call to the [DeductAdjustment ](../companion-api/api-reference-companion/remote/deductadjustment)method.


> 📘 Info
>
> A Deduct Adjustment is an adjustment being made that takes money from the cardholder, that has to be accepted by the wallet. This uses the [DeductAdjustment](../companion-api/api-reference-companion/remote/deductadjustment) method. Deduct Adjustments happens because a settlement wasn’t authorized, or was authorized for less.

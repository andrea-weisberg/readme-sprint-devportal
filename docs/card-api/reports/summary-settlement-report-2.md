...
- **Card Association Fee** – this can be either a debit or a credit transaction.
  - As a debit transaction, there is a fee paid to a card network for a specific service rendered.
  - As a credit transaction, there can be some discounts applied to the paid services.
  - There is a difference between Card Association Fee Credit and Card Association Fee Reversal. The latter refers to a reversal provided back to the issuer via an incorrect charge, whereas the former is a discount given off the fees.
...
[Daily*Settlement_Report_ICA*(YYYY_MM_DD).xls](https://developer.sprint.paymentology.com/wp-content/uploads/2023/12/Daily_Settlement_Report_ICA_YYYY_MM_DD-1.xls)

...
| tokens              | Array   | array – The tokens linked to this card, where each token has the following fields (see below for details). |
...
_Unmapped_. The token has not yet been linked to the Account PAN. The process of tokenization is In Progress.
_Active_. The token is linked to the Account PAN and may initiate new transactions to be authorized.
_Suspended_. The token is linked to the Account PAN but may not perform transactions at the request of one or more suspenders.
_Deleted_. The token is logically deleted but is still linked to the Account PAN for the purposes of post-authorization transaction processing. Eg. “Active”
